// https://www.tutorialspoint.com/cprogramming/c_bit_fields.htm --> Keep Learning.
...
//If you are using such variables inside a structure then you can define the width of
//a variable which tells the C compiler that you are going to use only those number of bytes.
...
3 width         The number of bits in the bit-field.
...
A bit field can hold more than a single bit; for example, if you need a variable to store a value from 0 to 7,
...
If you try to use more than 3 bits, then it will not allow you to do so. Let us try the following example −
*/
...
int main( ) {
   ...
   //As this uses more than 3 bits, then it will not allow the result to be accepted restarting the value as 0 by 8, 1 by 9
   //and so on, returning 0 as age in this case.
   Age.age = 8;
   printf( "Age.age : %d\n", Age.age );
