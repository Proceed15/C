// https://www.tutorialspoint.com/cprogramming/c_input_output.htm --> Keep Learning.
...
/**/
/*
When the above code is compiled and executed, it waits for you to input some text. When you enter a text and press enter,
then the program proceeds and reads only a single character and displays it as follows (using Command Line (CMD))−
$./a.out
...
//NOTE: Though it has been deprecated to use gets() function, Instead of using gets, use fgets() with more arguments.
/*
#include <stdio.h>
...
//When you enter a text and press enter, then the program proceeds and reads the complete line till end,
...
which means you have to provide valid inputs like "string integer". If you provide "string string" or "integer integer",
then it will be assumed as wrong input. Secondly, while reading a string,
scanf() stops reading as soon as it encounters a space, so "this is test" are three strings for scanf().
*/
