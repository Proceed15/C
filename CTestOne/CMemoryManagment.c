// https://www.tutorialspoint.com/cprogramming/c_memory_management.htm --> Keep Learning.
...
1 	    void *calloc(int num, int size);            This function allocates an array of num elements each of which
...
For example, to store a name of any person, it can go up to a maximum of 100 characters,
so you can define something as follows −

char name[100];

But now let us consider a situation where you have no idea about the length of the text you need to store,
...
good practice when you are not in need of memory anymore then you should release that memory by calling the function free().
...
int main() {
   ...
   if( description == NULL ) {
      ...
      strcat( description, "\vShe is in class 10th");
   }
...
You can try the above example without re-allocating extra memory,
and strcat() function will give an error due to lack of available
memory in description.
Memory that can be allocate with calloc and malloc and re-allocate with realloc.
*/
