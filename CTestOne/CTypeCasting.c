// https://www.tutorialspoint.com/cprogramming/c_type_casting.htm --> Keep Learning.
...
or it can be specified explicitly through the use of the cast operator. It is considered good programming practice to
...
main() {
   ...
   int  i = 19;
   char c = 'c'; // ascii value is 99 for c and 100 for d.
   int sum;
...
then they are converted to the type unsigned int and after that long going to one that
appears highest in the following hierarchy until it's matches −
...
usual arithmetic conversion applies and the compiler converts i and c into 'float' and adds them yielding a 'float' result.
*/
